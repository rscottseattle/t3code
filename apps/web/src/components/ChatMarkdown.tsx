...
const MarkdownFileLink = memo(function MarkdownFileLink({
  ...
  const showFileContextMenu = useCallback(
    async (position: { x: number; y: number }) => {
      ...
      const openWithChildren = environmentId
      ...
      try {
        const clicked = await api.contextMenu.show(
          [
            ...(canOpenInViewer ? ([{ id: "open-panel", label: "Open in panel" }] as const) : []),
...
function areMarkdownFileLinkPropsEqual(
  ...
  return (
    ...
    previous.availableEditors === next.availableEditors &&
...
function ChatMarkdown({
  ...
  return (
    <div
      className={cn(
        "chat-markdown w-full min-w-0 text-sm leading-relaxed text-foreground/80 [overflow-wrap:anywhere] [word-break:break-word] dark:text-white",
        className,
      )}
      onCopy={handleCopy}
    >
